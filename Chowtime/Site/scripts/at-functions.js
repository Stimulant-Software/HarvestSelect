...
function atLoadReport7Day() {
    ...
    $('#changeFarm').unbind().change(function () {
        ...
        $('#currentFarm').empty().text(farmName);
        if (dateEnd == "") dateEnd = getDate(1); // yesterday - reports shouldn't include "today"
        pondReport7Day(farmID, dateEnd);
    });

    $('#atReportDateEnding').unbind().blur(function () {
        
        dateEnd = $(this).val(); // may need adjustment for JSONing
        if (!farmID) { alert("Please select a farm") } else { pondReport7Day(farmID, dateEnd); }
    });
}
    ...
    $.ajax('../api/Pond/PondList', {
        ...
        success: function (msg) {
            ...
            $.when( $('#farmPonds').empty().html(farmTotalHtml) ).then(function () {
                bindButtons();
                searchQuery = { "key": _key, "userID": userID, "FarmId": farmID, "CurrentTime": dateEnd, "StatusId": "1" }, data = JSON.stringify(searchQuery);
...
function pondReport7Day(farmID, dateEnd) {
    showProgress('body', 'farmRepeater');
    if (!dateEnd) { dateEnd = new Date(); }
    var farmPondsHtml = '', resultsPonds = {}, searchQuery = { "key": _key, "userID": userID, "FarmId": farmID, "StatusId": "1" }, data = JSON.stringify(searchQuery);
	debugger;
	$.ajax('../api/Pond/PondList', {
        ...
        success: function (msg) {
            ...
            if (resultsPonds.length < 1) {
                ...
                for (var i = 0; i < resultsPonds.length; i++) {
                    farmPondsHtml += '<section class="pond" id="pond' + resultsPonds[i].PondId + '" data-pondid="' + resultsPonds[i].PondId + '">';
                    farmPondsHtml += reportRepeater7Day(resultsPonds[i]);
                    farmPondsHtml += '</section>';
                }
            }
            var farmTotalHtml = '<section ID="farmPonds">' + farmPondsHtml + '</section>';
                    ...
                    success: function (msg) {
                        ...
                        for (var i = 0; i < pondList.length; i++) {
                            ...
                            for (var j = 0; j < dateList.length; j++) {
                                var resultsList = msg['ReturnData'][pondList[i]][dateList[j]];
                                var pondID = pondList[i], date = dateList[j], element = j;
                                o2LevelsGraph7Day(pondID, date, resultsList, element);
                                hideProgress('farmRepeater');
                            }
                        }
                    }
...
function reportRepeater7Day(pondInfo) {
    var statusClass, pondHtml = '', day = 0;
    switch (pondInfo.StatusId) { case "1": statusClass = "good"; break; case "2": statusClass = "poor"; break; case "3": statusClass = "bad"; break; }
    pondHtml += '<section class="pond-name" data-status="' + statusClass + '"><span class="name">' + pondInfo.PondName + '</span></section>';
    pondHtml += '<section class="aeratorsHX"><section class="aeratorHX7Day" id="' + pondInfo.PondId + '_' + (day) + '"></section></section>';
    return pondHtml;
}
// END REPORT CODE
                    ...
                    success: function (msg) {
                        ...
                        for (var i = 0; i < pondList.length; i++) {
                            ...
                            for (var j = 0; j < dateList.length; j++) {
                                ...
                                var pondID = pondList[i], date = dateList[j], element = j;
                                if (j == 0) {
                                    o2LevelsGraph(pondID, date, resultsList, element, true);
                                } else {
                                    o2LevelsGraph(pondID, date, resultsList, element, false);
                                }
                            }
                        }
                        hideProgress('farmRepeater');
...
function o2LevelsGraph( pond, date, readings, element, getLastEntries) {
    ...
    if (readings.length == 0) {
        ...
        for (var i = 0; i < readings.length; i++) {
            portableArray[i] = parseInt(readings[i]['PortableCount'], 10);
            stationaryArray[i] = parseInt(readings[i]['StaticCount'], 10);
            o2Array[i] = parseFloat(readings[i]['O2Level']);
            timestampArray[i] = formatDateForHighcharts(readings[i]['ReadingDate']);
            if (i == readings.length - 1 && getLastEntries) {
                $('#pond' + pond + ' #currentO2').attr('placeholder', o2Array[i]);
                $('#pond' + pond + ' #currentStationaryArray').val(stationaryArray[i]);
                $('#pond' + pond + ' #currentPortableAerators').val(portableArray[i]);
            }
        }
        $(pondDOM).highcharts({
...
function o2LevelsGraph7Day(pond, date, readings, element, getLastEntries) {
    var myDate = new Date(date);
    
    var myDate2 = new Date(date);
    myDate2.setDate(myDate2.getDate() - 7);
    
    var titleText = myDate2.getMonth() + 1 + "/" + myDate2.getDate() + "/" + myDate2.getFullYear() + " - " + (myDate.getMonth() + 1) + "/" + myDate.getDate() + "/" + myDate.getFullYear(), pondDOM = "#" + pond + "_" + element, portableArray = [], stationaryArray = [], o2Array = [], timestampArray = [];
    //console.log(pondDOM);
    if (readings.length == 0) {
        $(pondDOM).append("<p>There are no results for " + date + "</p>");
    } else {
        for (var i = 0; i < readings.length; i++) {
            ...
            o2Array[i] = parseFloat(readings[i]['O2Level']);
            timestampArray[i] = formatDateForHighcharts7Day(readings[i]['ReadingDate']);
            if (i == readings.length - 1 && getLastEntries) {
                $('#pond' + pond + ' #currentO2').attr('placeholder', o2Array[i]);
        ...
        $(pondDOM).highcharts({
            ...
            yAxis: [ { labels: { format: '{value}', style: { color: '#89A54E' } }, title: { text: 'O2 Levels', style: { color: '#89A54E' } }, minRange: 14, min: 0, max: 14, tickInterval: 4 }], // Secondary yAxis - aerator count, both stationary and portable
            tooltip: { shared: true, headerFormat: '<b>{point.x}</b><br/>', pointFormat: 'O2 Levels:{point.y}', xDateFormat: "%m %d   %H:%M"},
            legend: { layout: 'vertical', align: 'left', x: 120, verticalAlign: 'top', y: 100, floating: true, backgroundColor: '#FFFFFF', enabled: false },
            credits: { enabled: false },
            plotOptions: { column: { stacking: 'normal' } },
            series: [
...
function bindButtons() {
    $('.at-entry .submit').unbind().click(function (e) {
        e.preventDefault();
        var pondID = $(this).parent().parent().data('pondid'), o2Input = $(this).parent().find('#currentO2'), o2Reading = o2Input.val() == "" ? "0" : o2Input.val(), aeratorsStationaryInput = $(this).parent().find('#currentStationaryAerators'), aeratorsStationary = aeratorsStationaryInput.val() == "" ? "0" : aeratorsStationaryInput.val(), aeratorsPortableInput = $(this).parent().find('#currentPortableAerators'), aeratorsPortable = aeratorsPortableInput.val() == "" ? "0" : aeratorsPortableInput.val(), currentPond = $(this).parent().parent().find('.aeratorsHX section:last'), now = new Date(), readingDate = now.toLocaleDateString() + " " + now.toLocaleTimeString().split(" ")[0] + " " + now.toLocaleTimeString().split(" ")[1], currentSection = $(this).parent().parent();
        showProgress(currentSection, 'submitATEntry');
        var searchQuery = { "key": _key, "ReadingDate": readingDate, "ReadingId": -1, "PondId": pondID, "O2Level": o2Reading, "StaticCount": aeratorsStationary, "PortableCount": aeratorsPortable }, data = JSON.stringify(searchQuery);
        $.ajax('../api/Pond/O2AddOrEdit', {
            ...
            success: function (msg) {
                ...
                if (now.getHours() < 12) { now.setDate(now.getDate() - 1); readingDate = now.toLocaleDateString(); } else { readingDate = now.toLocaleDateString(); }
                searchQuery = { "key": _key, "PondId": pondID, "ReadingDate": readingDate, "StatusId": "1" }, data = JSON.stringify(searchQuery);
                $.ajax('../api/Pond/PondO2ByDate', {
                    ...
                    success: function (msg) {
                        ...
                        o2LevelsGraph(pondID, today, resultsList, 0, true); // 0 refers to numeric part of DOM ID for pond
                        var statusClass;
                        switch (resultsList[0].PondStatus) { case "1": statusClass = "good"; break; case "2": statusClass = "poor"; break; case "3": statusClass = "bad"; break; }
                        $('#pond' + pondID + ' .pond-name').attr('data-status', statusClass);
...
function formatDateForHighcharts7Day(date) {
    //var timestamp = date.split(" ");
    return date;
}